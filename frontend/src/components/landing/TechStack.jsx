import { useReveal } from '../../hooks/useReveal.js';

const BADGES = [
  { emoji: '🎙',  name: 'LiveKit',     desc: 'Real-time audio & WebRTC' },
  { emoji: '🔊',  name: 'Deepgram',    desc: 'Voice recognition + TTS' },
  { emoji: '🧠',  name: 'OpenRouter',  desc: 'LLM reasoning & tool calls' },
  { emoji: '🗄',  name: 'Neon',        desc: 'Session memory (PostgreSQL)' },
  { emoji: '⚛️', name: 'React',       desc: 'Live kitchen UI' },
  { emoji: '🟢',  name: 'Node.js',     desc: 'Agent worker & API' },
];

const MARQUEE_ITEMS = [...BADGES, ...BADGES];

export default function TechStack() {
  const headerRef = useReveal();
  const badgesRef = useReveal();

  return (
    <section className="py-24 bg-stone-50">
      <div className="max-w-7xl mx-auto px-6">

        <div ref={headerRef} className="reveal text-center mb-14">
          <p className="text-amber-600 text-sm font-semibold uppercase tracking-widest mb-3">
            The stack
          </p>
          <h2 className="text-4xl md:text-[2.75rem] font-bold text-stone-900 leading-tight mb-4">
            Engineered on the best infrastructure.
          </h2>
          <p className="text-stone-600 text-lg max-w-xl mx-auto">
            Every layer chosen for reliability and speed.
          </p>
        </div>

        <div
          ref={badgesRef}
          className="reveal flex flex-wrap justify-center gap-4 mb-14"
        >
          {BADGES.map((b) => (
            <div
              key={b.name}
              className="glass-card flex items-center gap-3 px-5 py-3.5 rounded-full hover:border-amber-300 transition-colors duration-200 cursor-default"
            >
              <span className="text-xl" role="img" aria-label={b.name}>{b.emoji}</span>
              <div>
                <p className="text-sm font-semibold text-stone-900 m-0 leading-none">{b.name}</p>
                <p className="text-xs text-stone-500 m-0 mt-0.5 leading-none">{b.desc}</p>
              </div>
            </div>
          ))}
        </div>

        <div className="marquee-wrapper opacity-30 select-none" aria-hidden="true">
          <div className="marquee-track">
            {MARQUEE_ITEMS.map((b, i) => (
              <span key={i} className="text-sm font-medium text-stone-500 whitespace-nowrap flex items-center gap-3">
                <span>{b.emoji} {b.name}</span>
                <span className="opacity-40">·</span>
              </span>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
}
